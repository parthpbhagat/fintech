    ...
    <div className="fixed bottom-6 right-6 z-50">
      {!isOpen && (
        <button
          ...
          className="group relative flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-tr from-primary to-primary/80 text-white shadow-2xl transition-all hover:scale-110 active:scale-95"
        >
          ...
          <div className="border-t border-slate-100 bg-white p-4">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-xs font-medium outline-none focus:border-primary transition-all"
              />
              <Button 
                onClick={handleSend}
                disabled={isLoading}
                className="rounded-xl bg-primary hover:bg-primary/90 h-10 w-10 p-0 flex items-center justify-center shadow-lg shadow-primary/20"
              >
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {["Check Risk", "Summarize Charges"].map(tag => (
                <button 
                  ...
                  className="rounded-full bg-slate-100 px-3 py-1 text-[9px] font-bold text-slate-500 uppercase hover:bg-primary/10 hover:text-primary transition-colors"
                >
                  {tag}
                </button>
              ))}
